...
run_scMerge<-function(count,cellinfo,former.meth=''){
  ...
  colnames(cellinfo)[colnames(cellinfo)=='Batch']='batch'
  sob <- CreateSeuratObject(counts = count)
  # sob <- NormalizeData(sob)
  sob@assays$RNA@data=LogNormalize(sob@assays$RNA@counts)
  ...
  processed <- res@assays@data$scMerge_res 
  
  res_name<-paste0(ifelse(former.meth=='','',paste0(former.meth,'+')),'scmerge')
  colnames(cellinfo)[colnames(cellinfo)=='batch']='Batch'
