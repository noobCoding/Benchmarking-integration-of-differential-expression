ref_df<-read.delim(paste0(standard_positive_dir,'C0152013_disease_gda_summary.tsv'))
ref_df%<>%dplyr::filter(Score_gda>=0.3)
ref_df%<>%dplyr::arrange(desc(Score_gda))
ref_genes.1<-ref_df$Gene
ref_weight.1<-ref_df$Score_gda
ref_df<-read.delim(paste0(standard_positive_dir,'CTD_D000077192_genes_20210713220641.tsv'))
