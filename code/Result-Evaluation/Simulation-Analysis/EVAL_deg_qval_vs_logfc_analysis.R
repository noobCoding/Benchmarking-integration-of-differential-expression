...
for (simu in vect_simu){
  ...
  # result.list<-readRDS(paste0("data/7batch.simul6_dropout_39_full.RData"))
  ...
  for (method in vect_method){
    ...
    print("singinficant deg")
    
    # all DEGs
    S3 <- S3[rownames(S3) %in% DEG$x,]
    # 
    logfc_count <- c(logfc_count, sum(abs(S3$log2FC) > 0.5))
    qval_count <- c(qval_count, sum(S3$adj.pval <= 0.05))
    both_count <- c(both_count, sum((S3$adj.pval <= 0.05) & (abs(S3$log2FC) > 0.5) ))
  }
  
  
  logfc_perc<- c(logfc_perc,100*round(logfc_count/ length(DEG$x), 4))
  qval_perc <- c(qval_perc, 100*round(qval_count / length(DEG$x), 4))
  both_perc <- c(both_perc, 100*round(both_count / length(DEG$x), 4))
  
  logfc_perc[is.na(logfc_perc)] = 0
  qval_perc[is.na(qval_perc)] = 0
  both_perc[is.na(both_perc)] = 0
  
  df <- data.frame(percent=perc, percent_005=perc_005, percent_qval=perc_qval,
                   both_count= both_count, both_perc = both_perc,
                   logfc_count=logfc_count, logfc_perc = logfc_perc,
                   qval_count=qval_count, qval_perc = qval_perc,
                   count=hist, simu=simu, method=method)
...
# rawmedian = median(F_all$both_count[F_all$method=='Raw_Wilcox'])
# rawmedian = rep(median(rawmedian), length(F_all$method))
# F_all$RawMedian = as.numeric(rawmedian)
# df <- ddply(F_all, .(both_count), summarise, median=median(RawMedian, na.rm = TRUE))
# 
# p4 <- ggplot(F_all, aes(x=method, y=both_count, color=method)) + scale_color_manual(values = method_col)+
...
#   labs(y = 'DEGs with abs(logFC)>0.5 and q-value≤0.05') + theme(axis.text.y = element_text(size=16, color = a))

t = length(unique(F_all$method))
rawmedian = median(F_all$both_perc[F_all$method=='Raw_Wilcox'])
rawmedian = rep(median(rawmedian), length(F_all$method))
F_all$RawMedian = as.numeric(rawmedian)
df <- ddply(F_all, .(both_perc), summarise, median=median(RawMedian, na.rm = TRUE))

p4 <- ggplot(F_all, aes(x=method, y=both_perc, color=method)) + scale_color_manual(values = method_col)+
  geom_boxplot(outlier.size = 1) + coord_flip() +
  geom_hline(data=df, aes(yintercept=median),linetype="dashed", color='black') +
  labs(y = 'Percentage of DEGs: abs(logFC)>0.5 & q-value≤0.05') + theme(axis.text.y = element_text(size=16, color = a))

# t = length(unique(F_all$method))
# rawmedian = median(F_all$qval_count[F_all$method=='Raw_Wilcox'])
# rawmedian = rep(median(rawmedian), length(F_all$method))
# F_all$RawMedian = as.numeric(rawmedian)
# df <- ddply(F_all, .(qval_count), summarise, median=median(RawMedian, na.rm = TRUE))
# 
# p4 <- ggplot(F_all, aes(x=method, y=qval_count, color=method)) + scale_color_manual(values = method_col)+
#   geom_boxplot(outlier.size = 1) + coord_flip() +
#   geom_hline(data=df, aes(yintercept=median),linetype="dashed", color='black') +
#   # labs(y = 'F-score') + theme(axis.text.y = element_text(size=14, color = a))
#   labs(y = 'DEGs with q-value≤0.05') + theme(axis.text.y = element_text(size=16, color = a))

# t = length(unique(F_all$method))
# rawmedian = median(F_all$qval_perc[F_all$method=='Raw_Wilcox'])
# rawmedian = rep(median(rawmedian), length(F_all$method))
# F_all$RawMedian = as.numeric(rawmedian)
# df <- ddply(F_all, .(qval_perc), summarise, median=median(RawMedian, na.rm = TRUE))
# 
# p4 <- ggplot(F_all, aes(x=method, y=qval_perc, color=method)) + scale_color_manual(values = method_col)+
#   geom_boxplot(outlier.size = 1) + coord_flip() + #ylim(c(0, 15))+
#   geom_hline(data=df, aes(yintercept=median),linetype="dashed", color='black') +
#   # labs(y = 'F-score') + theme(axis.text.y = element_text(size=14, color = a))
#   labs(y = 'Percentage of DEGs with q-value≤0.05') + theme(axis.text.y = element_text(size=16, color = a))
...
# t = length(unique(F_all$method))
# rawmedian = median(F_all$logfc_perc[F_all$method=='Raw_Wilcox'])
# rawmedian = rep(median(rawmedian), length(F_all$method))
# F_all$RawMedian = as.numeric(rawmedian)
# df <- ddply(F_all, .(logfc_perc), summarise, median=median(RawMedian, na.rm = TRUE))
# 
# p4 <- ggplot(F_all, aes(x=method, y=logfc_perc, color=method)) + scale_color_manual(values = method_col)+
#   geom_boxplot(outlier.size = 1) + coord_flip() +
#   geom_hline(data=df, aes(yintercept=median),linetype="dashed", color='black') +
#   # labs(y = 'F-score') + theme(axis.text.y = element_text(size=14, color = a))
#   labs(y = 'Percentage of DEGs with abs(logFC)>0.5') + theme(axis.text.y = element_text(size=16, color = a))

# t = length(unique(F_all$method))
# rawmedian = median(F_all$logfc_count[F_all$method=='Raw_Wilcox'])
# rawmedian = rep(median(rawmedian), length(F_all$method))
# F_all$RawMedian = as.numeric(rawmedian)
# df <- ddply(F_all, .(logfc_count), summarise, median=median(RawMedian, na.rm = TRUE))
# 
# p4 <- ggplot(F_all, aes(x=method, y=logfc_count, color=method)) + scale_color_manual(values = method_col)+
#   geom_boxplot(outlier.size = 1) + coord_flip() + #ylim(c(0, 200))+
#   geom_hline(data=df, aes(yintercept=median),linetype="dashed", color='black') +
#   # labs(y = 'F-score') + theme(axis.text.y = element_text(size=14, color = a))
#   labs(y = 'DEGs with abs(logFC)>0.5') + theme(axis.text.y = element_text(size=16, color = a))
