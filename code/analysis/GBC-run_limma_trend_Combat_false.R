...
for(base_name in base_names){
  ...
  counts<- read.table(file = paste0('demo_Combat/',x2, '/all/output.txt'), sep = "\t", fill=T)
  cellinfo<-read.table(file = paste0(base_name,"/cellinfo.txt"), sep = "\t", fill=T)
  count_df<-counts
  ...
  lmfit <- eBayes(lmfit, trend=FALSE, robust = TRUE)
  ...
  write.table(result.table, file=paste0(base_name,"/limma_trend_combat_result_false.txt"), sep = "\t")
}
