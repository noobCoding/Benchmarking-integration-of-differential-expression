...
for(base_name in base_names){
  ...
  counts<- read.table(file = paste0('demo_MNN/',x2, '/all/output.txt'), sep = "\t", fill=T)
  cellinfo<-read.table(file = paste0(base_name,"/cellinfo.txt"), sep = "\t", fill=T)
  count_df<-counts
  ...
  write.table(result.table, file=paste0(base_name,"/limma_trend_MNN_result_false.txt"), sep = "\t")
}
