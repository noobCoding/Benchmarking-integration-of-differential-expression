...
run_FEM<-function(res, cellinfo,former.meth=''){
  ...
  res<-data.frame(mu.hat=mu.hat,pval=res.temp$pval,FDR=res.temp$FDR,row.names=rownames(ES))
  save(res, cellinfo, file=paste0('./',ifelse(former.meth=='','',paste0(former.meth,'_sep','+')),'FEM.rda'))
}