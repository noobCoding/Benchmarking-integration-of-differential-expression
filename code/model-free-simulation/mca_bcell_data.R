...
mycell = which(metadata$orig.ident=='B-cell')
...
b1.markers %>% group_by(cluster) %>% top_n(n = 2, wt = avg_log2FC)
...
b2.markers %>% group_by(cluster) %>% top_n(n = 2, wt = avg_log2FC)
...
for (K in K_intervals)
{
  ...
  ds = paste0("mca_bcell_", cutoff,"_b1", a, b,"_", pp,"_", K)
  dir.create(ds)
