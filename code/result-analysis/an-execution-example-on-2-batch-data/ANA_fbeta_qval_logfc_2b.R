...
p4 <- ggplot(F_all, aes(x=Var2, y=F.score, color=Var2)) + scale_color_manual(values = Var2_col)+
  geom_boxplot(outlier.size = 1) + coord_flip() + ylim(c(0, 0.8))+
  geom_hline(data=df, aes(yintercept=median),linetype="dashed", color='black') + 
  labs(y = 'F-score (beta=0.5): abs(logFC)>0.5 and q-value≤0.05') + theme(axis.text.y = element_text(size=16, color = a))
  # labs(y = 'F-score (beta=0.5): q-value≤0.05') + theme(axis.text.y = element_text(size=16, color = a))
...
p5
pdf("2b_LBE_pval_logfc.pdf", width=10, height=10)
p5
dev.off()